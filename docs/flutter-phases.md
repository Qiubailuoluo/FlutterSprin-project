...
| 3 | 首页统计卡片 + 分类列表 | 已完成 |
| 4 | 账单列表/表单 CRUD | 已完成 |
| 5 | 完善交互、错误提示、UI 组件库（可选 TDesign） | 已完成 |

## 阶段 5 验证

1. 操作成功/失败时 SnackBar 样式统一（绿/红/默认）
2. 列表加载失败显示「重试」按钮；空列表有图标提示
3. 登录/注册错误显示红色提示条
4. Token 失效后自动退出并提示「登录已过期」
5. `flutter analyze` + `flutter test`

详见 [learn/12-flutter-ux-polish.md](learn/12-flutter-ux-polish.md)
